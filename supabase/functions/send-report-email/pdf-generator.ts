...
if (!supabaseUrl || !supabaseAnonKey) {
  console.error("Erro crítico: SUPABASE_URL ou SUPABASE_ANON_KEY não definidos");
} else {
...
export async function generatePdf(auditoriaId: string): Promise<Uint8Array> {
  try {
    ...
    // Fetch audit data
    console.log("Buscando dados da auditoria...");
    ...
    console.log("Dados da auditoria obtidos:", JSON.stringify({
      id: auditoria.id,
    ...
    console.log(`${perguntas?.length || 0} perguntas encontradas`);
    ...
    console.log(`${secoes?.length || 0} seções encontradas`);

    // Generate HTML content for the report
    console.log("Gerando conteúdo HTML do relatório...");
...
function generateReportHtml(data: any) {
  try {
    console.log("Gerando HTML do relatório com dados:", JSON.stringify({
      loja: data.loja?.nome,
      dataAuditoria: data.auditoria?.data,
      ...
      <body>
        <div class="header">
          <div>
            ...
            <p>Loja: ${data.loja?.nome || 'N/A'} (${data.loja?.numero || 'N/A'})</p>
            <p>Data: ${data.auditoria?.data ? new Date(data.auditoria.data).toLocaleDateString('pt-BR') : 'N/A'}</p>
          </div>
        </div>
  ...
  try {
    ...
    secoes.forEach(secao => {
      try {
        ...
        console.error("Erro ao processar seção:", secao.id, error);
        html += `
          <tr>
            <td>${secao.nome || 'Erro'}</td>
            <td>Erro ao calcular</td>
          </tr>
        `;
      }
    });
    
    return html || '<tr><td colspan="2">Nenhuma seção encontrada</td></tr>';
  } catch (error) {
    console.error("Erro ao gerar pontuação por seções:", error);
    return '<tr><td colspan="2">Erro ao gerar pontuação</td></tr>';
  }
}
  ...
  try {
    ...
    itensCriticos.forEach(item => {
      try {
        ...
        console.error("Erro ao processar item crítico:", item.id, error);
      }
    });
    ...
    console.error("Erro ao gerar pontos de atenção:", error);
    return "<p>Erro ao gerar pontos de atenção.</p>";
  }
}
